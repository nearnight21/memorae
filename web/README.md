...
当前阶段验证 VMK V1 的本地体验和本机密文同步，不连接正式国内服务器，也不会改写现有线上记忆或照片。项目默认使用不可用的后端占位配置，避免误连接 ThinkPad 的 Supabase 和 R2。
...
`src/sync/` 已接入默认 VMK 验证页面，可人工上传或下载本地密文服务中的钥匙信封、记忆和
照片密文。服务地址和访问令牌只保存在页面内存；自动测试也会让 Android 与 Web 双向上传、
下载并解密密文。运行方法见
