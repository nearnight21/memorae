...
默认根页面（`/`）是“所忆”官方 Landing Page（包含产品特性介绍、高保真 Mac/手机拟物视窗动效与 Android APK 下载）。
访问 `/#app`（或 `?app=1`）可直接进入正式 Web 应用（账号登录、私密空间门禁与真实足迹地图）。本地开发模式下，页面左下角提供一键切换悬浮按钮。
在正式应用中，所有数据先加密后保存在浏览器 IndexedDB。
