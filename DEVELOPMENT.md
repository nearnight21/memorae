...
- App 时间轴底盘改为向下、向两侧渐淡的完整弧面，静止时保留轮廓与相邻年份；年份透明度
  与底盘显隐独立。首页底部柔化层修正为覆盖地图的绝对定位层，减弱地名对年份阅读的干扰。
  本轮保留中心按钮的圆环、尺寸、阴影、位置及手势代码，不改变年份选择和回中逻辑。
  验证：App `verify`（138 项测试、Expo Doctor 21 项）、运行面边界检查及 `git diff --check`
  通过；arm64 standalone 包已覆盖安装到无线调试设备。真机外观核对尚待首页保持前台。
- App 设备解锁扩展到 iOS：iOS 文案显示“面容 / 指纹”、Android 显示“指纹”，两端共用同一套
