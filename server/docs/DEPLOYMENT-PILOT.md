...
```text
memory-recall/v1/{account-id}/photos/{photo-id}/content.json
```

客户端不得自行指定完整 COS 路径，也不得拿到长期 COS 密钥。为了保持现有 `/v1/photos/:id` 的请求和响应结构不变，首轮将 `EncryptedPhotoV1.content` 原样序列化为 UTF-8 JSON 后存入 COS，并由 API 在读取时与数据库中的 `metadata_json` 重新组合。后续需要降低 Base64 开销时，再通过新的协议版本改为二进制直传。对象内容已被客户端加密，但仍按私有用户数据处理：禁止公共读、禁止 CDN 公共缓存、禁止在日志中写出对象内容。
...
2. 增加账号密码、会话校验和跨账号隔离测试。
3. 实现 PostgreSQL 存储及迁移脚本，保留 JSON 存储仅供本地测试。
