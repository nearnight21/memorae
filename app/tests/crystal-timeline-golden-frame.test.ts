...
test('正式中心年份按钮以互斥手势承载回到现在、上拉创建和下拉回到全景', async () => {
  ...
  assert.match(timeline, /resolveResetPullRelease/);
  assert.match(timeline, /onResetMapView/);
  assert.match(timeline, /ARC_TIMELINE_GESTURE_RESET_MAP/);
  ...
  assert.match(home, /onResetMapView=\{onResetMapView\}/);
  assert.match(home, />回到全景<\/Animated\.Text>/);
