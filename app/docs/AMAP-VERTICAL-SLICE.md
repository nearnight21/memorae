...
中文译名使用以 GeoNames id 为键的受控离线映射，不在运行时调用在线翻译。首轮覆盖东京、巴黎、纽约、伦敦及其验收周边城市；5,642 条 GeoNames 原始数据仍完整保留，缺少可靠中文名的城市不会进入地图标签候选，避免主视图中英文混杂。普通浏览、`zoom < 8`、北京等境内目标均清空标签；海外记忆和地点选取分别使用更克制与更完整的阈值，并按目标距离、首都和人口排序。测试壳中点击海外记忆 Marker 或海外照片聚类会下钻并自动开启记忆上下文，点击东京/巴黎/纽约入口会自动开启地点选取上下文，切换城市或关闭详情会清除相应标签且不重建 `MapView`。
